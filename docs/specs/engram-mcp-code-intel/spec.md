...
- [x] `search` returns code symbols via `KnowledgeQuery` (entity name/kind substring match, routes through the provider). The `LexicalFeed` remains wired for future BM25 ranking once an entity-id resolver lane is added.
