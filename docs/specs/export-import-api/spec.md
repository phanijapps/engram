...
The exported `ImportData` reuses the existing import record types, so export → `dry_run_import` → `apply_import` round-trips for single-backend scope-to-scope movement. v1 covers the families whose concrete stores expose listing methods (knowledge + memory); hierarchy/belief export is deferred where no list method exists. Contradictions are not in the round-trip (ImportData has no contradictions vec).
...
- [x] v1 export covers knowledge (sources/documents/chunks/entities/relationships) + memory; hierarchy/belief/concept-schemes/concepts/vectors deferred (no concrete scope-wide list method). Contradictions are not in the round-trip (`ImportData` has no contradictions vec). Export is a non-atomic best-effort read (concurrent writes between list calls can yield an inconsistent snapshot). (deferred: `export-import-hierarchy-belief`)
...
- Technical: the concrete stores expose listing for knowledge (`list_entities`/`list_sources`/`list_documents`/`list_chunks` at `adapters/knowledge/sqlite/src/service.rs`) and memory (`list_memories` at `adapters/memory/sqlite/src/service.rs`); hierarchy/belief have no scope-wide list method today (source: grep).
