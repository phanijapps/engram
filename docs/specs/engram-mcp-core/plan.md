...
- 2026-07-29: T4 split into **T4a** (config + read tools — done) / **T4b** (persist config into `OntologyRepository`/`TaxonomyRepository` for governance — deferred; not AC-required, since the `OntologyRepository` trait has no list methods, so `ontology_read` returns the active in-memory config). Tools-batch review fixes: AC #4 `lanes` implemented as a `RetrievalTargetType` post-filter on `recall`; `put_entity` rejects unknown `kind`s (was silently defaulting `Concept`); required-field validation (`-32602`) on every write tool; `forget` rejects unknown modes; relationship IDs use a unit-separator-delimited tuple (no collision); `recall` optional `limit`; dropped 3 unused deps. Per-group module split + `register_all` relocation deferred to T6 (when `tools.rs` outgrows one module).
