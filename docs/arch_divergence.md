...
| Hierarchy construction/navigation separation | Distinct Construction Module and Navigation Module ("Organize then Retrieve") (`architecture-design-v2.md:159-164`); durable chunk hierarchy in the structured tier (`architecture-design-v2.md:535`). | In-memory + durable SQLite backends behind `HierarchyRepository` (navigation) and `HierarchyBuilder` (construction) — distinct ports in `engram-hierarchy`; the `path_for` traversal is shared via `engram_hierarchy::navigation` (extracted 2026-07-01). | Semantic clustering and model-assisted summaries are deferred (research open decisions); construction and navigation share one crate (the trait-level split is sufficient). | Add clustering/model-assisted summaries when proactive hierarchy building is prioritized. | `80%` |
...
| Consolidation as a formal, separately-owned pipeline | A separate pipeline with explicit inputs, outputs, conflict handling, and evaluation (`synthesis.md:224-226`). | `engram-consolidation` owns `ConsolidationService`, the dry-run + gated mutating services, and the planning/validation helpers, with auditable `ConsolidationRun` reports (RFC-0006 / ADR-0010). | Consolidation is no longer anchored in `engram-core`; the trigger policy is decided (**ADR-0011** — explicit-command baseline; automatic triggers deferred); additional task algorithms remain future work. | Implement additional consolidation task algorithms; introduce an auto-trigger scheduler via a new ADR if needed. | `85%` |
...
| Hierarchical memory and HiRAG | In-memory hierarchy build/navigation/retrieval expansion plus a durable SQLite `HierarchyRepository` (`engram-store-hierarchy-sqlite`); `path_for` traversal shared via `engram_hierarchy::navigation`. | Semantic clustering and model-assisted summaries deferred. |
...
5. **✅ Dispatch the Cue and Temporal retrieval modes** (done, 2026-07-01 — `docs/specs/temporal-cue-retrieval`; the in-memory adapter routes both, cue matches memory links, temporal scores by recency).
