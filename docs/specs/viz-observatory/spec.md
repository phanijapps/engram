...
- [x] The Observatory tab is the interactive 2D graph explorer — a toolbar
  (highlight search + top-N density + refresh) + the `viz-foundation` deck.gl
  overview canvas (no D3) with drill + a bottom LearningHealthBar whose
  belief/hierarchy items open detail slideovers (honest empty-states today). The
  Graph tab is a separate animated 3D globe overview (a distinct view, not a
  duplicate).
